...
public:
	...
	void DrawGrid(vector<double> y_first_graph, int typefunc, vector<double> x_first_graph, int N)
	{
		if (!init) return;

		CPen subgrid_pen(PS_DOT, 1, RGB(200, 200, 200)); //ручка для рисования подсетки
		CPen grid_pen(PS_SOLID, 1, RGB(0, 0, 0));		//ручка для рисования сетки
		...
		data_y_min = *min_element(y_first_graph.begin(), y_first_graph.end());
		data_y_max = ceil(*max_element(y_first_graph.begin(), y_first_graph.end()) * 1.1);
	...
	void DrawPlot(vector<double>& y_first_graph, int typefunc, vector<double>& x_first_graph, int N)
	{
		if (!init) return;
